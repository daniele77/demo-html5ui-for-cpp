...
int main()
{
    ...
    handlers["connection"] = [&](const json& event)
    {
        auto server = event.at("server");
        auto user = event.at("user");
        auto password = event.at("password");
    ...
    handlers["call"] = [&](const json& event)
    {
        const std::string number = event.at("number");
        softPhone.Call(number);
    };
    
    handlers["answer"] = [&](const json& event)
    {
        softPhone.Answer();
    };

    handlers["hangup"] = [&](const json& event)
    {
        softPhone.Hangup();
    };

    handlers["autoanswer"] = [&](const json& event)
    {
        const std::string value = event.at("value");
        softPhone.Autoanswer(value == "yes");
    };

    handlers["agc"] = [&](const json& event)
    {
        const std::string value = event.at("value");
        softPhone.AGC(value == "yes");
    };

    handlers["volume"] = [&](const json& event)
    {
        const std::string value = event.at("value");
        softPhone.Volume(std::stoul(value));
    };
    ...
    ws.RequestHandler(
        [&](const std::string& req)
        {
            ...
            try
            {
                ...
                auto request = jsonObj.at("request");

                auto handler = handlers.find(request);
